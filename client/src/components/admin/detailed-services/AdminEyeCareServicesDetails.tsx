...
const AdminEyeCareServicesDetails: FC<Props> = ({ service }) => {
    const [activeTab, setActiveTab] = useState<string>('Overview');
    const [isAddingPartner, setIsAddingPartner] = useState<boolean>(false);

    const [partnersList, setPartnersList] = useState<EyePartner[]>([
        { id: 1, name: 'Apollo Eye Clinic', location: 'Bengaluru, KA', services: 'Cataract, Lasik, Glaucoma, General Checkup', vendorPrice: '₹1,200/Avg', salePrice: '₹1,600/Avg', active: true, img: 'https://images.unsplash.com/photo-1581594693702-fbdc51b2763b?w=400' },
        { id: 2, name: 'Vasan Eye Care', location: 'Chennai, TN', services: 'General Checkup, Retina, Cornea', vendorPrice: '₹900/Avg', salePrice: '₹1,300/Avg', active: true, img: 'https://images.unsplash.com/photo-1519494026892-80bbd2d6fd0d?w=400' },
        { id: 3, name: 'Centre for Sight', location: 'Delhi, DL', services: 'Lasik, Pediatric Ophthalmology', vendorPrice: '₹2,500/Avg', salePrice: '₹3,200/Avg', active: false, img: 'https://images.unsplash.com/photo-1579684385127-1ef15d508118?w=400' },
    ]);

    const [requestsList, setRequestsList] = useState<EyeRequest[]>([
        { id: 'EYE-REQ-001', patient: 'Ravi Kumar', clinic: 'Apollo Eye Clinic', slot: 'Today, 04:30 PM', type: 'Lasik Consult', status: 'Pending' },
        { id: 'EYE-REQ-002', patient: 'Sneha Sharma', clinic: 'Vasan Eye Care', slot: 'Tomorrow, 10:00 AM', type: 'General Checkup', status: 'Pending' },
        { id: 'EYE-REQ-003', patient: 'Amit Patel', clinic: 'Apollo Eye Clinic', slot: 'Today, 02:15 PM', type: 'Cataract Screening', status: 'Approved' },
    ]);

    const detailKpis: KPI[] = [
        { title: 'Clinic Partners', value: partnersList.length.toString(), trend: '+3', icon: <FiMapPin />, color: 'text-purple-600', bg: 'bg-purple-50' },
        { title: 'Appointments (This Month)', value: '1,285', trend: '+14%', icon: <FiCalendar />, color: 'text-purple-600', bg: 'bg-purple-50' },
        { title: 'Pending Requests', value: requestsList.filter(r => r.status === 'Pending').length.toString(), trend: '-2%', icon: <FiClock />, color: 'text-orange-500', bg: 'bg-orange-50' },
        { title: 'Revenue Generated', value: '₹14.2 L', trend: '+16%', icon: <FiTrendingUp />, color: 'text-emerald-600', bg: 'bg-emerald-50' },
    ];

    const tabs: string[] = ['Overview', 'Eye Clinic Partner', 'Eye Request', 'History', 'Settings'];

    const handleCreatePartner = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        
        const formData = new FormData(e.currentTarget);
        const newPartner: EyePartner = {
            id: Date.now(),
            ...
            active: true,
            img: 'https://images.unsplash.com/photo-1581594693702-fbdc51b2763b?w=400'
        };
        ...
        <div className="p-6 md:p-8 grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50/30 animate-in fade-in duration-300">
            <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex flex-col min-h-[250px]">
                ...
                <div className="flex-1 relative w-full mt-2">
                    <div className="absolute left-0 top-0 bottom-6 flex flex-col justify-between text-[9px] text-gray-400 font-semibold">
                        <span>400</span><span>200</span><span>0</span>
                    </div>
                    <div className="absolute left-6 right-0 top-0 bottom-6">
                        <svg viewBox="0 0 100 40" className="w-full h-full" preserveAspectRatio="none">
                            ...
                            <path d="M0,35 Q15,15 25,20 T50,10 T75,25 T100,5 L100,40 L0,40 Z" fill="url(#purpleGradient)" />
                            <path d="M0,35 Q15,15 25,20 T50,10 T75,25 T100,5" fill="none" stroke="#9333ea" strokeWidth="1.5" />
                        </svg>
                    </div>
                </div>
            </div>
            <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex flex-col">
                ...
                <div className="flex-1 space-y-4 mt-2">
                    {[
                        { name: 'General Eye Checkup', val: 80, count: '540' },
                        { name: 'Cataract Screening', val: 65, count: '310' },
                        { name: 'LASIK Consultation', val: 45, count: '240' },
                        { name: 'Glaucoma Test', val: 30, count: '195' }
                    ].map((serv, i) => (
            ...
            return (
                <div className="p-6 md:p-8 animate-in fade-in duration-300 max-w-4xl mx-auto">
                    <div className="flex items-center gap-3 border-b border-gray-100 pb-5 mb-6">
                        ...
                        <div>
                            <h3 className="font-bold text-gray-900 text-[18px]">Onboard Eye Clinic Partner</h3>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <div className="md:col-span-2">
                                    <Input 
                                        ...
                                        required 
                                        name="clinicName" 
                                        placeholder="e.g., Apollo Eye Clinic" 
                                        theme="purple" 
                                    />
                                </div>
                                <div className="space-y-1.5 md:col-span-2">
                                    ...
                                    <textarea required name="services" rows={2} placeholder="e.g., Cataract, Lasik, Glaucoma, General Checkup..." className="w-full bg-gray-50 border border-gray-200 text-gray-900 text-[13px] rounded-xl px-4 py-2.5 focus:bg-white focus:ring-2 focus:ring-purple-600/20 focus:border-purple-600 outline-none transition-all placeholder-gray-400 resize-none" />
                                </div>
                                <Input 
    ...
    const mockHistory: EyeHistory[] = [
        { id: 'CHK-EYE-881', date: '15 May 2024', patient: 'Rahul Gupta', clinic: 'Vasan Eye Care', treatment: 'General Checkup', userType: 'Corporate' },
        { id: 'CHK-EYE-882', date: '12 May 2024', patient: 'Priya Sharma', clinic: 'Apollo Eye Clinic', treatment: 'Lasik Follow-up', userType: 'Insurance' },
        { id: 'CHK-EYE-883', date: '05 May 2024', patient: 'Amit Patel', clinic: 'Centre for Sight', treatment: 'Cataract Screening', userType: 'Individual' },
    ];
    ...
    const RenderHistory: FC = () => (
        <div className="p-6 animate-in fade-in duration-300">
            <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-5 border-b border-gray-50 pb-5">
                <h3 className="font-bold text-gray-900 text-[15px]">Completed Eye Checkups</h3>
            ...
            <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="flex items-center justify-between bg-gray-50 p-4 rounded-xl border border-gray-200">
                        ...
                        <ToggleSwitch theme="purple" defaultChecked />
                    </div>
                    <div className="flex items-center justify-between bg-gray-50 p-4 rounded-xl border border-gray-200">
    ...
    return (
        <div className="bg-white rounded-[2rem] border border-gray-100 shadow-[0_4px_30px_-10px_rgba(0,0,0,0.05)] overflow-hidden animate-in fade-in slide-in-from-bottom-8 duration-500">
            <div className="p-6 md:p-8 border-b border-gray-100 flex flex-col lg:flex-row lg:items-start justify-between gap-6">
                ...
                <div className="flex items-center gap-3">
                    <Button variant="neutral" icon={<FiExternalLink className="w-4 h-4" />}>View Eye Portal</Button>
            ...
            <div>
                ...
                {activeTab === 'Eye Clinic Partner' && <RenderPartners />}
                {activeTab === 'Eye Request' && <RenderRequests />}
...
export default AdminEyeCareServicesDetails;