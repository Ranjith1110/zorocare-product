...
import type { ServiceData } from '../AdminManageServiceDetails';
...
const AdminDentalServicesDetails: FC<Props> = ({ service }) => {
  const [activeTab, setActiveTab] = useState<string>('Overview');
  const [isAddingPartner, setIsAddingPartner] = useState<boolean>(false);

  const [partnersList, setPartnersList] = useState<DentalPartner[]>([
    { id: 1, name: 'Apollo Dental Clinic', location: 'Bengaluru, KA', services: 'Root Canal, Implants, Braces, Whitening', vendorPrice: '₹1,500/Avg', salePrice: '₹2,000/Avg', active: true, img: 'https://images.unsplash.com/photo-1606811841689-23dfddce3e95?w=400' },
    { id: 2, name: 'Clove Dental', location: 'Mumbai, MH', services: 'General Checkup, Cleaning, Extraction', vendorPrice: '₹800/Avg', salePrice: '₹1,200/Avg', active: true, img: 'https://images.unsplash.com/photo-1598256989800-fea5f95b5cf4?w=400' },
    { id: 3, name: 'Smile Care Experts', location: 'Delhi, DL', services: 'Orthodontics, Cosmetic Dentistry', vendorPrice: '₹2,500/Avg', salePrice: '₹3,500/Avg', active: false, img: 'https://images.unsplash.com/photo-1609840114035-3c981b782dfe?w=400' },
  ]);

  const [requestsList, setRequestsList] = useState<DentalRequest[]>([
    { id: 'DNT-REQ-001', patient: 'Ravi Kumar', clinic: 'Apollo Dental Clinic', slot: 'Today, 04:30 PM', type: 'Root Canal Consult', status: 'Pending' },
    { id: 'DNT-REQ-002', patient: 'Sneha Sharma', clinic: 'Clove Dental', slot: 'Tomorrow, 10:00 AM', type: 'Teeth Whitening', status: 'Pending' },
    { id: 'DNT-REQ-003', patient: 'Amit Patel', clinic: 'Apollo Dental Clinic', slot: 'Today, 02:15 PM', type: 'General Checkup', status: 'Approved' },
  ]);

  const detailKpis: KPI[] = [
    { title: 'Clinic Partners', value: partnersList.length.toString(), trend: '+2', icon: <FiMapPin />, color: 'text-blue-600', bg: 'bg-blue-50' },
    { title: 'Appointments (This Month)', value: '1,532', trend: '+12%', icon: <FiCalendar />, color: 'text-blue-600', bg: 'bg-blue-50' },
    { title: 'Pending Requests', value: requestsList.filter(r => r.status === 'Pending').length.toString(), trend: '-5%', icon: <FiClock />, color: 'text-orange-500', bg: 'bg-orange-50' },
    { title: 'Revenue Generated', value: '₹18.5 L', trend: '+18%', icon: <FiTrendingUp />, color: 'text-purple-600', bg: 'bg-purple-50' },
  ];

  const tabs: string[] = ['Overview', 'Dental Clinic Partner', 'Dental Request', 'History', 'Settings'];

  const handleCreatePartner = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();

    const formData = new FormData(e.currentTarget);
    const newPartner: DentalPartner = {
      id: Date.now(),
      ...
      active: true,
      img: 'https://images.unsplash.com/photo-1606811841689-23dfddce3e95?w=400'
    };
    ...
    <div className="p-6 md:p-8 grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50/30 animate-in fade-in duration-300">
        ...
        <div className="flex-1 relative w-full mt-2">
          ...
          <div className="absolute left-6 right-0 top-0 bottom-6">
            <svg viewBox="0 0 100 40" className="w-full h-full" preserveAspectRatio="none">
              ...
              <path d="M0,35 Q15,15 25,20 T50,10 T75,25 T100,5 L100,40 L0,40 Z" fill="url(#blueGradient)" />
              <path d="M0,35 Q15,15 25,20 T50,10 T75,25 T100,5" fill="none" stroke="#2563eb" strokeWidth="1.5" />
            </svg>
          </div>
        </div>
      </div>
      <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex flex-col">
        ...
        <div className="flex-1 space-y-4 mt-2">
          {[
            { name: 'General Checkup & Cleaning', val: 85, count: '650' },
            { name: 'Root Canal Treatment', val: 60, count: '412' },
            { name: 'Teeth Whitening', val: 45, count: '280' },
            { name: 'Dental Implants', val: 25, count: '190' }
          ].map((serv, i) => (
            <div key={i} className="flex items-center gap-3">
              <span className="text-[11px] font-bold text-gray-700 w-44 truncate">{serv.name}</span>
              <div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden">
                <div className="h-full bg-blue-600 rounded-full" style={{ width: `${serv.val}%` }}></div>
              </div>
      ...
      return (
        <div className="p-6 md:p-8 animate-in fade-in duration-300 max-w-4xl mx-auto">
          <div className="flex items-center gap-3 border-b border-gray-100 pb-5 mb-6">
            <Button variant="neutral" className="!p-2" onClick={() => setIsAddingPartner(false)}>
              <FiArrowLeft className="w-4 h-4" />
            </Button>
            <div>
              <h3 className="font-bold text-gray-900 text-[18px]">Onboard Dental Clinic Partner</h3>
              <p className="text-[12px] text-gray-500 font-medium mt-0.5">Add a new clinic, list provided services, and configure pricing margins</p>
            </div>
          </div>
          ...
          <form onSubmit={handleCreatePartner} className="space-y-6">
            <div className="bg-white border border-gray-200 rounded-2xl p-6 space-y-5 shadow-sm">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div className="space-y-1.5 md:col-span-2">
                  ...
                  <textarea required name="services" rows={2} placeholder="e.g., Root Canal, Implants, Braces, Whitening..." className="w-full bg-gray-50 border border-gray-200 text-gray-900 text-[13px] rounded-xl px-4 py-2.5 focus:bg-white focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600 outline-none transition-all placeholder-gray-400 resize-none" />
                </div>
                <Input
            ...
            <div className="bg-gray-50 border-2 border-dashed border-gray-300 rounded-2xl p-8 flex flex-col items-center justify-center text-center hover:bg-blue-50/50 hover:border-blue-500/50 transition-colors cursor-pointer group">
              ...
              <h3 className="text-[15px] font-bold text-gray-900 mb-1">Upload Clinic Image</h3>
              <p className="text-[12px] text-gray-500 font-medium mb-4">High resolution image (16:9 ratio recommended)</p>
              <Button variant="secondary" className="!text-blue-600 !border-blue-200 hover:!bg-blue-50 pointer-events-none">Browse Files</Button>
            </div>
          ...
          <table className="w-full text-left border-collapse min-w-[950px]">
            <thead>
              <tr className="bg-gray-50/50 border-b border-gray-100">
                <th className="px-5 py-4 text-[11px] font-bold text-gray-500 uppercase tracking-wider">Clinic Details</th>
              ...
              {partnersList.map((p, i) => (
                <tr key={i} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-5 py-4">
                    ...
                    <div className="text-[11px] text-gray-500">Sale: <span className="font-bold text-blue-600">{p.salePrice}</span></div>
                  </td>
                  <td className="px-5 py-4 text-center">
                    <ToggleSwitch defaultChecked={p.active} theme="blue" />
                  </td>
                  <td className="px-5 py-4 text-center">
                    <Button variant="neutral" className="!p-1.5 text-gray-400 hover:text-blue-600 inline-flex">
        ...
        <table className="w-full text-left border-collapse min-w-[900px]">
          <thead>
            <tr className="bg-gray-50/50 border-b border-gray-100">
              ...
              <th className="px-5 py-4 text-[11px] font-bold text-gray-500 uppercase">Selected Clinic</th>
              <th className="px-5 py-4 text-[11px] font-bold text-gray-500 uppercase">Treatment Type</th>
          ...
          <tbody className="divide-y divide-gray-50">
            {requestsList.map((req, i) => (
              <tr key={i} className="hover:bg-gray-50/50 transition-colors">
                <td className="px-5 py-4 text-[12px] font-bold text-blue-600">{req.id}</td>
                ...
                <td className="px-5 py-4 text-center">
                  {req.status === 'Pending' ? (
                    <Button
                      variant="primary"
                      className="!bg-blue-600 hover:!bg-blue-700 !px-3 !py-1.5 !text-[11px]"
  ...
  const mockHistory: DentalHistory[] = [
    { id: 'CHK-DNT-881', date: '15 May 2024', patient: 'Rahul Gupta', clinic: 'Clove Dental', treatment: 'General Cleaning', userType: 'Corporate' },
    { id: 'CHK-DNT-882', date: '12 May 2024', patient: 'Priya Sharma', clinic: 'Apollo Dental Clinic', treatment: 'Root Canal', userType: 'Insurance' },
    { id: 'CHK-DNT-883', date: '05 May 2024', patient: 'Amit Patel', clinic: 'Apollo Dental Clinic', treatment: 'Consultation', userType: 'Individual' },
  ];
  ...
  const RenderHistory: FC = () => (
    <div className="p-6 animate-in fade-in duration-300">
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-5 border-b border-gray-50 pb-5">
        <h3 className="font-bold text-gray-900 text-[15px]">Completed Dental Checkups</h3>
      ...
      <div className="w-full overflow-x-auto border border-gray-100 rounded-2xl scroll-smooth [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        <table className="w-full text-left border-collapse min-w-[900px]">
          <thead>
            <tr className="bg-gray-50/50 border-b border-gray-100">
              <th className="px-5 py-4 text-[11px] font-bold text-gray-500 uppercase tracking-wider">Checkup ID</th>
              ...
              <th className="px-5 py-4 text-[11px] font-bold text-gray-500 uppercase tracking-wider">Clinic Partner</th>
              <th className="px-5 py-4 text-[11px] font-bold text-gray-500 uppercase tracking-wider">Treatment</th>
            ...
            {mockHistory.map((h, i) => (
              <tr key={i} className="hover:bg-gray-50/50 transition-colors">
                ...
                <td className="px-5 py-4 text-[12px] text-center">
                  <span className={`px-2.5 py-1 rounded text-[10px] font-bold tracking-wide ${h.userType === 'Corporate' ? 'bg-indigo-50 text-indigo-600' : h.userType === 'Insurance' ? 'bg-orange-50 text-orange-600' : 'bg-gray-100 text-gray-600'}`}>
                    {h.userType}
                  </span>
                </td>
              </tr>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="flex items-center justify-between bg-gray-50 p-4 rounded-xl border border-gray-200">
            ...
            <ToggleSwitch theme="blue" defaultChecked />
          </div>
          <div className="flex items-center justify-between bg-gray-50 p-4 rounded-xl border border-gray-200">
            <div><span className="text-[13px] text-gray-800 font-bold block">Require Advance Payment</span><span className="text-[11px] text-gray-500">For individual user types</span></div>
            <ToggleSwitch theme="blue" defaultChecked />
          </div>
          <div className="flex items-center justify-between bg-gray-50 p-4 rounded-xl border border-gray-200">
            <div><span className="text-[13px] text-gray-800 font-bold block">Send Reminder SMS/WhatsApp</span><span className="text-[11px] text-gray-500">24 hours before appointment</span></div>
            <ToggleSwitch theme="blue" defaultChecked />
          </div>
          <div className="flex items-center justify-between bg-gray-50 p-4 rounded-xl border border-gray-200">
            <div><span className="text-[13px] text-gray-800 font-bold block">Post-Consultation Follow-up</span><span className="text-[11px] text-gray-500">Send feedback link to user</span></div>
            <ToggleSwitch theme="blue" defaultChecked />
          </div>
        </div>
      </div>
      <div className="flex justify-end">
        <Button variant="primary" className="!bg-blue-600 hover:!bg-blue-700" icon={<FiCheckCircle />}>Save Operations Settings</Button>
      </div>
    </div>
  );
    ...
    <div className="bg-white rounded-[2rem] border border-gray-100 shadow-[0_4px_30px_-10px_rgba(0,0,0,0.05)] overflow-hidden animate-in fade-in slide-in-from-bottom-8 duration-500">
      <div className="p-6 md:p-8 border-b border-gray-100 flex flex-col lg:flex-row lg:items-start justify-between gap-6">
        <div className="flex gap-5">
          <div className={`w-16 h-16 rounded-2xl flex items-center justify-center shrink-0 shadow-sm border border-blue-100 bg-blue-50 text-blue-600`}>
        ...
        <div className="flex items-center gap-3">
          <Button variant="neutral" icon={<FiExternalLink className="w-4 h-4" />}>View Dental Portal</Button>
        ...
        <div className="flex gap-8">
          {tabs.map((tab) => (
            <button
              ...
              className={`py-4 text-[13px] font-bold whitespace-nowrap transition-colors border-b-2 ${activeTab === tab ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-800'}`}
            >
              {tab}
            </button>
          ))}
      ...
      <div>
        ...
        {activeTab === 'Dental Clinic Partner' && <RenderPartners />}
        {activeTab === 'Dental Request' && <RenderRequests />}
...
export default AdminDentalServicesDetails;