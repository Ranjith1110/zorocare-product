        ...
        <div className="w-full space-y-6 animate-in fade-in duration-500 pb-10">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {topKPIs.map((kpi, idx) => (
                    <div key={idx} className="bg-white p-4 sm:p-5 rounded-md border border-gray-100 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.03)] flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left gap-3 sm:gap-4 hover:shadow-md transition-shadow">
                        <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-md flex items-center justify-center shrink-0 ${kpi.bg} ${kpi.color}`}>
                            {kpi.icon}
                        </div>
                        <div>
                            <p className="text-[9px] sm:text-[10px] font-bold text-gray-500 mb-1 uppercase tracking-wider">{kpi.title}</p>
                            <h3 className="font-heading text-lg sm:text-xl font-bold text-gray-900 leading-none">{kpi.value}</h3>
                            <p className={`text-[9px] font-bold flex items-center justify-center sm:justify-start gap-0.5 mt-1.5 ${kpi.isPositive ? 'text-emerald-500' : 'text-red-500'}`}>
                                <FiArrowUpRight className="w-3 h-3" /> {kpi.trend} <span className="text-gray-400 font-medium font-sans ml-0.5 hidden sm:inline">vs last month</span>
                            </p>
                        </div>
                    </div>
                ))}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5 sm:gap-6">
                    {valueProps.map((prop, idx) => (
                        <div key={idx} className="flex items-center gap-4">
                            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-[#5301ab]/10 flex items-center justify-center shrink-0">
                                {prop.icon}
                            </div>
                            <div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-stretch">
                ...
                <div className="xl:col-span-4 bg-gray-50/80 rounded-md border border-gray-100 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.03)] p-6 md:p-8 flex flex-col relative overflow-hidden h-full min-h-[300px] sm:min-h-[400px]">
                    ...
                    <div className="absolute -right-4 -bottom-4 w-[70%] sm:w-[60%] xl:w-[90%] pointer-events-none drop-shadow-2xl opacity-50 sm:opacity-100">
